...
for(i in 1:length(allcountries)) {
  ...
  country_oubreaks <- mcr1_df[which(mcr1_df$Country.Codes == curr_country_code), ]
  ...
  matpoints(seq(1, time_span, 1), realPrevalences, pch = 4)
  ...
  runSimulation <- function(new_epsilon) {
    ...
    return(calculateDistance(simulated_values, realPrevalences))
  }
