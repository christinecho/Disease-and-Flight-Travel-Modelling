...
for(i in 1:length(allcountries)) {
  ...
  C = 1e-20
  if (curr_country_code == "CHN") next
  X = as.integer(C * S * subset(pop_data, iso3c == "CHN")$value * subset(mean_predic, origin_code == "CHN" & destination_code == curr_country_code)$PredMu)
  S = as.integer(N-W-X)
  epsilon = 0.05
  rt = 1/7
  rx = 1/13
  ...
  runSimulation <- function(x) {
    params = list(beta = beta, rt = rt, rx = rx, rw = rw, epsilon = x[1], rnovo = rnovo, N=N);
    init.values = c(
      S = S,   # susceptible humans
      W = W,   # infected wild type humans
      X = as.integer(x[2]* S * subset(pop_data, iso3c == "CHN")$value * subset(mean_predic, origin_code == "CHN" & destination_code == curr_country_code)$PredMu))   
    ...
    print(calculateDistance(simulated_values, year_freq$Freq))
  ...
  # ABC model
  tolerance = c(160, 80, 40, 20, 10, 5, 2.5, 1)/1000
  epsilon_sample = list(c("unif", 0, 1))
  c_sample = list(c("unif", 0, 1e-19))
  ABC_Beaumont <- ABC_sequential(method="Beaumont", model=runSimulation, prior=c(epsilon_sample, c_sample), 
  ...
  for (i in 1:length(tolerance)) {
    ...
    new_Csp = median(ABC_Beaumont[["intermediary"]][[i]][["posterior"]][,3])
  ...
  text(time_span/2, max(r[, "W"])/2 - max(r[,"W"])/4, new_Csp)
}